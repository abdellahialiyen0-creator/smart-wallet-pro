...
const BudgetModal = React.memo(() => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center p-2 sm:p-4 z-[100]">
            ...
            <motion.div
                ...
                className="bg-white border rounded-3xl md:rounded-[2.5rem] shadow-2xl w-full max-w-4xl z-10 relative overflow-hidden flex flex-col"
                style={{
            ...
            >
                {/* الشريط العلوي للموبايل / الجانبي للديسكتوب */}
                <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
                    {/* الشريط الجانبي (Sidebar) - يصبح أفقي على الموبايل */}
                    <div
                        className="w-full md:w-64 border-b md:border-b-0 md:border-l transition-colors duration-500 flex-shrink-0"
                        style={{
                            backgroundColor: isDark ? '#1e293b' : '#f8fafc',
                            borderColor: 'var(--app-border)'
                        }}
                    >
                        {/* Header - يظهر فقط على الديسكتوب */}
                        <div className="hidden md:flex items-center gap-3 p-6 md:p-8">
                        ...
                        {/* التبويبات - أفقية على الموبايل، عمودية على الديسكتوب */}
                        <nav className="flex md:flex-col gap-2 p-3 md:px-6 md:py-0 overflow-x-auto md:overflow-x-visible custom-scrollbar">
                            {tabs.map(tab => (
                                <button
                                    key={tab.id}
                                    onClick={() => setActiveTab(tab.id)}
                                    className={`flex items-center gap-2 md:gap-3 px-3 md:px-4 py-2.5 md:py-3 rounded-xl md:rounded-2xl font-bold text-[10px] md:text-xs transition-all whitespace-nowrap ${activeTab === tab.id
                        ...
                        {/* أزرار الحفظ والإلغاء - تظهر فقط على الديسكتوب */}
                        <div className="hidden md:flex flex-col gap-3 p-6 md:p-8 mt-auto">
                            <motion.button
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 md:p-10 relative">
                        ...
                        <AnimatePresence mode="wait">
                            {activeTab === 'appearance' && (
                                ...
                                >
                                    <div>
                                        <h4 className="text-lg md:text-xl font-black mb-2" style={{ color: 'var(--app-text)' }}>مظهر التطبيق</h4>
                                        <p className="text-[10px] md:text-xs text-slate-400 font-bold">اختر الثيم الذي يناسب ذوقك من مجموعتنا الحصرية</p>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 md:gap-4">
                                        {Object.values(THEMES).map((t) => (
                                            <button
                                                ...
                                                className={`group relative p-3 md:p-4 rounded-2xl md:rounded-[2rem] border-2 transition-all flex flex-col items-center gap-2 md:gap-3 overflow-hidden ${theme === t.id
                                            ...
                                            >
                                                <div
                                                    className="w-full h-12 md:h-16 rounded-xl md:rounded-2xl shadow-lg transition-transform group-hover:scale-105"
                                                    style={{ background: t.preview }}
                                                ></div>
                                                <span className="text-[9px] md:text-[10px] font-black" style={{ color: 'var(--app-text)' }}>{t.name}</span>
                            ...
                            {activeTab === 'currency' && (
                                ...
                                >
                                    <div>
                                        <h4 className="text-lg md:text-xl font-black mb-2" style={{ color: 'var(--app-text)' }}>العملة المفضلة</h4>
                                        <p className="text-[10px] md:text-xs text-slate-400 font-bold mb-4 md:mb-6">سيتم تحويل كافة المبالغ والميزانيات تلقائياً للعملة المختارة</p>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 md:gap-3">
                                            {Object.values(CURRENCIES).map((curr) => (
                                                <button
                                                    ...
                                                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl border-2 flex items-center justify-between transition-all ${currency === curr.code
                                                ...
                                                >
                                                    <div className="flex flex-col items-start gap-0.5 md:gap-1">
                                                        <span className="text-base md:text-lg">{curr.flag}</span>
                                                        <span className="text-[9px] md:text-[10px] font-black" style={{ color: 'var(--app-text)' }}>{curr.name}</span>
                                                    </div>
                            ...
                            {activeTab === 'budgets' && (
                                ...
                                >
                                    <div>
                                        <h4 className="text-lg md:text-xl font-black mb-2" style={{ color: 'var(--app-text)' }}>تخطيط الميزانية الذكية</h4>
                                        <p className="text-[10px] md:text-xs text-slate-400 font-bold">حدد سقف المصاريف لكل فئة لتصلك تنبيهات عند الاقتراب من الرقم</p>
                                    </div>
                                    ...
                                    <div className="grid gap-3">
                                        {Object.keys(CATEGORIES).filter(cat => CATEGORIES[cat].type === 'expense').map(cat => (
                                            <div
                                                key={cat}
                                                className="flex items-center justify-between p-3 md:p-4 rounded-2xl md:rounded-3xl border border-slate-50 dark:border-slate-800 bg-slate-50/30 dark:bg-slate-900/20 hover:border-blue-200 transition-all group"
                                                style={{ borderColor: 'var(--app-border)' }}
                                            >
                                                <div className="flex items-center gap-2 md:gap-4">
                                                    <div className="text-lg md:text-2xl p-2 md:p-3 bg-white dark:bg-slate-800 rounded-xl md:rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 transition-transform group-hover:scale-110" style={{ borderColor: 'var(--app-border)' }}>
                                                        {CATEGORIES[cat].icon}
                                                    </div>
                                                    <span className="font-black text-[10px] md:text-xs" style={{ color: 'var(--app-text)' }}>{CATEGORIES[cat].name}</span>
                                                </div>
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        placeholder="0"
                                                        className="w-20 md:w-32 border p-2 md:p-3 rounded-xl md:rounded-2xl text-center font-black text-[10px] md:text-xs focus:ring-4 focus:ring-blue-500/10 outline-none transition-all"
                                                        style={{
                            ...
                            {activeTab === 'data' && (
                                ...
                                >
                                    <div>
                                        <h4 className="text-lg md:text-xl font-black mb-2" style={{ color: 'var(--app-text)' }}>البيانات والأمان</h4>
                                        <p className="text-[10px] md:text-xs text-slate-400 font-bold">تحكم في خصوصية بياناتك المخزنة محلياً على جهازك</p>
                                    </div>
                    ...
                    {/* أزرار الحفظ والإلغاء للموبايل - تظهر في الأسفل */}
                    <div className="flex md:hidden flex-col gap-2 p-4 border-t" style={{ borderColor: 'var(--app-border)', backgroundColor: isDark ? '#1e293b' : '#f8fafc' }}>
                        <motion.button
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            onClick={() => { setShowBudgetModal(false); toast.success('تم حفظ التغييرات'); }}
                            className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 text-white rounded-xl font-black text-xs shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all"
                            style={{ backgroundColor: 'var(--app-accent)', boxShadow: '0 10px 15px -3px rgba(var(--app-accent-rgb, 59, 130, 246), 0.3)' }}
                        >
                            <Save className="w-4 h-4" />
                            <span>حفظ التغييرات</span>
                        ...
                        <button
                            onClick={() => setShowBudgetModal(false)}
                            className="w-full flex items-center justify-center gap-2 py-2.5 text-[10px] font-black text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/20 rounded-xl transition-all"
                        >
                            إلغاء وتراجع
                        </button>
                    </div>
